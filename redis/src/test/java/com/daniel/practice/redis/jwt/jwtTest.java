...
public class jwtTest {
	...
	void customHeaderExtract() {
		SecretKey key = Jwts.SIG.HS256.key().build();
		String jwt = Jwts.builder()
			.header().add("customHeader", "customValue").and()
			.signWith(key)
			.compact();

		Jws<Claims> parsed = Jwts.parser().verifyWith(key).build().parseSignedClaims(jwt);
	...
	void requireSubject() {
		...
		// 올바른 subject
		Jwts.parser().verifyWith(key).requireSubject("user123").build().parseSignedClaims(jwt);

		// 잘못된 subject
		try {
			Jwts.parser().verifyWith(key).requireSubject("other").build().parseSignedClaims(jwt);
	...
	void jweEncryptDecrypt() {
		...
		String subject = Jwts.parser().decryptWith(key).build().parseEncryptedClaims(jwt).getPayload().getSubject();
